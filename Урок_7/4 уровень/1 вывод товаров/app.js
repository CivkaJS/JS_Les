...
function clickHandler(event) {
    ...
    let length_del = box.children.length;

    for (let i = 0; i < length_del; i++) {
...
function getProductMarkup(product) {
    ...
    for (let i = 0; i < product.length; i++) {
        box.insertAdjacentHTML('beforeend', `<div>${product[i].name}</div>`);
        box.insertAdjacentHTML('beforeend', `<img src="${product[i].imageUrl}" alt="${product[i].name}">`);
        box.insertAdjacentHTML('beforeend', `<div>${product[i].price}</div>`);
        box.insertAdjacentHTML('beforeend', `<a href="https://example.com/producs/${product[i].id}">Подробнее</a>`);
    }
}
